          ...
          <div class="newspaper-category">
            ...
            <template v-for="(article, key) in author.editorials" v-if="key <= 5">
              <div :class="'cell cell--' + (key + 1)">
                ...
                <span v-if="key === 5" class="w-full text-left text-base font-bold leading-none font-sans ltt-headline mb-8"><nuxt-link class="red-block inline-block" :to="'/author/' + author.slug + '/'">Read more Contributions<br /><span class="text-white opacity-75 text-xs">from {{ author.author.name }}</span></nuxt-link></span>
              </div>
            </template>
            <template v-for="num in 6">
              <template v-if="author.editorials.length < num">
